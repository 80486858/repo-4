...
const useWorkerFunc = (func, options = defaultOptions$1) => {
  ...
  const [workerStatus, setWorkerStatus] = react.useState(WorkerStatus.IDLE);
  const worker = react.useRef();
  const promise = react.useRef(defaultPromise);
  ...
  const generateWorker = react.useCallback(() => {
    const workerUrl = createWorkerBlobUrl$1(func, remoteDependencies, transferable);
...
const useWorker = (func, options = defaultOptions) => {
  ...
  const postMessage2 = react.useCallback(
    (...funcArgs) => {
      var _a;
  ...
  const workerHook = react.useMemo(
    () => ({
...
exports.TRANSFERABLE_TYPE = TRANSFERABLE_TYPE;
exports.WorkerStatus = WorkerStatus;
exports.useWorker = useWorker;
exports.useWorkerFunc = useWorkerFunc;
exports.useWorkerState = useWorkerState;
